...
/// `crate::net::headers::is_cross_origin_isolated`). It drives `self.crossOriginIsolated`
/// and gates SAB postMessage transfer to workers.
///
/// `is_secure_context` is a per-document flag derived from the document
/// URL scheme (https/wss/file or http://localhost — see
/// `crate::js_runtime::is_secure_url`). It drives `self.isSecureContext` and gates
...
#[op2]
#[string]
pub fn op_get_profile_value(state: &mut OpState, #[string] key: &str) -> String {
    ...
    match &state.profile {
        Some(p) => match key {
            ...
            "device_class" => match p.device_class {
                crate::stealth::DeviceClass::Desktop => "Desktop",
                crate::stealth::DeviceClass::MobileAndroid => "MobileAndroid",
                crate::stealth::DeviceClass::MobileIOS => "MobileIOS",
            }
            .to_string(),
            _ => String::new(),
        },
...
#[op2(fast)]
pub fn op_is_secure_context(state: &mut OpState) -> bool {
    let state = state.borrow::<StealthState>();
...
/// buffer (`__bo_input_events.mouse`) with statistically correct
...
pub fn op_behavior_mouse_trajectory(
    ...
    let profile = crate::stealth::behavior::BehaviorProfile::default();
    let points = crate::stealth::behavior::mouse_trajectory(
        (from_x, from_y),
        (to_x, to_y),
        target_w,
        &profile,
