...
fn ensure_patches_applied(config: &Config) -> io::Result<()> {
    ...
    // Chrome 152 and 153 send a byte-identical ClientHello (same JA4
    // t13d1517h2_8daaf6152771_cb7bf5808d99, verified against both releases), so
    // one patch covers them; it is named for the newest version it tracks.
    println!("cargo:warning=applying browser_oxide chrome153 patch to boringssl");
    apply_patch(config, "browser-oxide-chrome153.patch")?;
...
fn main() {
   ...
   if config.target_os == "windows" {
    ...
    let mut builder = bindgen::Builder::default()
        // bindgen MSRV is 1.70, so this is enough. `RustTarget::stable` is the
        // non-deprecated spelling of the `Stable_1_68` constant: same target.
        .rust_target(
            bindgen::RustTarget::stable(68, 0).expect("1.68 is a supported bindgen target"),
        )
        .derive_copy(true)
        .derive_debug(true)
        .derive_default(true)
