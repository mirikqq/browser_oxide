...
- **Struct (source of truth):** `crates/browser_oxide/src/stealth/profile.rs`
- **Loader:** `crates/browser_oxide/src/stealth/config.rs`
- **Worked example file:** `crates/browser_oxide/profiles/chrome_148_macos.yaml`
- **Coherent references:** `crates/browser_oxide/src/stealth/presets.rs`
...
`crates/browser_oxide/profiles/chrome_148_macos.yaml` is the canonical worked
...
| `gpu_profile` | `GpuProfile` | **(optional)** Full GPU catalog entry (extensions, getParameter values, shader precision). Defaults to an NVIDIA RTX 3060 profile. See `crates/browser_oxide/src/stealth/gpu.rs` for catalog entries. |
...
See `crates/browser_oxide/profiles/chrome_148_macos.yaml` for the full example.
...
`ConfigError` variants (`crates/browser_oxide/src/stealth/config.rs`):
